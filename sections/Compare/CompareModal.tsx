...
const Especificacoes=({PCs}:Props)=>{
  ...
  return(
    <div className='w-full text-white border border-[#1b1b1b]  bg-[#141414]'>
      ...
      <div className={`w-full transition-[max-height] duration-500 ease-in-out overflow-hidden ${isOpen ? 'max-h-[999px]' : 'max-h-0'}`}>
        <div className='text-xs re1:text-base items-center p-[8px] grid grid-cols-5 gap-7 bg-[#1b1b1b]'>
          <p>Placa de Vídeo</p>
          {PCs.map(pc=><p className='line-clamp-3'>{pc.placaVideo}</p>)}
        </div>
        <div className='text-xs re1:text-base items-center p-[8px] grid grid-cols-5 gap-7'>
          <p>Processador</p>
          {PCs.map(pc=><p className='line-clamp-3'>{pc.processador}</p>)}
        </div>
        <div className='text-xs re1:text-base items-center p-[8px] grid grid-cols-5 gap-7 bg-[#1b1b1b]'>
          <p>Memória</p>
          {PCs.map(pc=><p className='line-clamp-3'>{pc.memoria}</p>)}
        </div>
        <div className='text-xs re1:text-base items-center p-[8px] grid grid-cols-5 gap-7'>
          <p>Armazenamento</p>
          {PCs.map(pc=>
          <p className='line-clamp-3'>{(pc.armazenamento.includes('HD') || pc.armazenamento.includes('SSD')) ? 
            pc.armazenamento: `${pc.tipoArm} ${pc.armazenamento}`}
          </p>)}
        </div>
        <div className='text-xs re1:text-base items-center p-[8px] grid grid-cols-5 gap-7 bg-[#1b1b1b]'>
          <p>Fonte</p>
          {PCs.map(pc=><p className='line-clamp-3'>{pc.fonte}</p>)}
        </div>
        <div className='text-xs re1:text-base items-center p-[8px] grid grid-cols-5 gap-7'>
          <p>Garantia</p>
    ...
    <>
      ...
      {isOpen && (
        <div className='fixed inset-0 flex items-center justify-center z-40 text-white'>
          ...
          <div className='overflow-y-hidden re1:overflow-y-scrol absolute flex flex-col gap-10 p-10 re1:p-12 h-[90vh] re1:h-[70vh] w-[90%] re1:w-4/5 bg-[#1e1e1e] ml-auto re1:m-auto rounded-lg z-[41]'>
            <div className='flex justify-center items-center pb-3 fixed'>
              <p className='text-xl re1:text-3xl font-bold'>Comparação dos Produtos</p>
              <button className='absolute -right-[15%] -top-[50%] cursor-poiter btn btn-sm btn-circle bg-[#3d3d3d] border-transparent' onClick={closeModal}>✕</button>
            </div>
            <div className='flex flex-col gap-10 relative re1:static left-0 w-max re1:w-full top-20 overflow-auto'>

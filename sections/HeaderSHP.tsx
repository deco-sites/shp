...
const HeaderSHP=({categ=[], logoUrl='', cta=[]}:Props)=>{
  return(
    <div className="w-full h-28 flex flex-row bg-[#000000]  items-center px-7">
      ...
      {categ.map((cat)=>(
        <div className="font-bold max-h-2 w-fit text-white text-base mx-4">{cat.name}</div>
      ))}
      ...
      <div className="w-26">
        <label className="flex gap-1">
          ...
          <button>
            <img src="https://shopinfo.vteximg.com.br/arquivos/icon-search.png" alt="lupinha" />
          </button>
        </label>
      </div>
      ...
      {cta.map((btn)=>(
        <div className="w-6 text-base flex gap-1 mx-2">
          ...
          <p className="text-white font-bold hover:text-[#3d3d3d] cursor-pointer">{btn.name}</p>
        </div>
      ))}
    </div>
  )

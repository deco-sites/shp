...
const Filtro=({filtro}:Props)=>{
  ...
  return(
    <div id='filterDiv' className='w-full flex flex-col bg-base-100 re1:bg-[#1e1e1e] border border-[#1e1e1e] re1:border-0'>
      ...
      <div className={`${open ? 'max-h-[340px]' : 'max-h-0'} trasition-[max-height] overflow-hidden duration-500 ease-in-out`}>
        <label className={`${filtro.values.length>2 ? 'flex' : 'hidden'} justify-around items-center py-1`}>
        ...
        <ul ref={valuesList} className={`flex flex-col gap-2 bg-[#141414] overflow-y-auto max-h-[300px] re1:scrollbar-shp`}>
          {filtro.values.map(filter=>
            <li className='py-1 px-2'>
              <label className='flex justify-start gap-2 cursor-pointer items-center'>
                <input id='filter' type='checkbox' name={filtro.label} value={filter.value} className='checkbox checkbox-primary checkbox-xs rounded-none [--chkfg:transparent]'/>

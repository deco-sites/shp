  ...
  return(
    <>
      <label className='w-[45%]'>
        ...
        <button className='bg-transparent border border-secondary w-full h-12 px-10 rounded-lg' onClick={openModal}>
          <Image className='mx-auto' src='https://shopinfo.vteximg.com.br/arquivos/filterIcon.png' width={35} height={30}/>
        </button>
      </label>

      <dialog ref={modal} className='bg-base-100 min-h-full min-w-[100vw] overflow-x-hidden overflow-y-auto'>
        <form>
          ...
          <div className='flex flex-col py-5 items-center gap-10 text-secondary'>
            ...
            <ul className='w-full'>
              {filters.map((filtro,index)=>
                <li className='text-lg font-bold text-secondary border-y border-y-[#1e1e1e] py-3 px-4' data-index={index} data-value={filtro.value} data-fq={filtro.fqType}
                  onClick={()=>closeModal()}
                >
                  {filtro.name}
                </li>
              )}
              <li className='text-lg font-bold text-secondary border-y border-y-[#1e1e1e] py-3 px-4' data-index={777} data-value='' data-fq=''
                onClick={()=>closeModal()}
              >
                Ver Todos
              </li>

...
const CardProd=(props:CardProps)=>{
  ...
  return (
  <a href={props.linkProd} className='flex flex-col re1:flex-row w-full re1:h-[215px] bg-[#171717] hover:shadow-[0_10px_25px_0] hover:shadow-[rgba(0,0,0,.85)]
  ...
  >
    ...
    <div className='flex re1:block re1:w-[15%]'>
      <div className='w-[32%] re1:w-full re1:h-full flex items-center justify-center'>
        <Image src={props.imgUrl} width={180} height={180} loading='lazy' decoding='sync' fetchPriority='low' className='w-[78%] re1:w-full' alt={props.prodName} title={props.prodName}/>
      </div>
    ...
    <div className='flex flex-col re1:flex-row items-center w-full re1:w-[55%]'>
      ...
      {props.brinde && (
        <div className='flex items-center justify-start re1:justify-between w-full re1:w-[50%] gap-[10px] px-[10px]'>
          <Image src={props.brinde.imageUrl} width={90} height={90}/>
          <div>
            <p className='text-xs'>{props.brinde.productName}</p>
            <p className='py-2 flex gap-1 justify-start items-center'>
              ...
              <span className='font-bold text-xs text-secondary capitalize'>Brinde Exclusivo</span>
            </p>
          </div>
        </div>
      )}
      ...
      <div className='flex flex-col items-center re1:items-start justify-center p-[10px] w-full re1:w-[40%] h-full'>
        ...
        <div className='bg-primary text-sm w-full rounded-lg font-extrabold flex gap-1 items-center justify-center h-7 py-1 mb-4'>
...
const Card=({product, frete, brinde, descontoPix}:Props)=>{
  ...
  if(product.additionalProperty!.some(propValue=>propValue.propertyID==='10' && propValue.name==='category')){
    ...
    return <FinalCardPC 
      ...
      brinde={brinde ? {imageUrl:brinde.ImageUrl as string, productName:brinde.ProductName as string} : {imageUrl:'https://shopinfo.vteximg.com.br/arquivos/icone-brinde-compre-ganhe 1.png', productName:'BRINDE'}}
    />
  }else{
    return <CardProd
      parcelas={maxInstallments}

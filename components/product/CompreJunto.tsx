...
const CompreJunto=({page}:Props)=>{
  ...
  const ProductA=({temSlide}:{temSlide:boolean})=>{
    return(
      <div className={`flex w-[50%] ${temSlide && 'relative left-[5%] re1:static'} re1:w-1/2 items-center justify-between`}>
          ...
          return (
            <Slider.Item index={index} className='carousel-item flex flex-col re1:flex-row gap-2 w-full items-center'>
              ...
              <div className='flex-col'>
                <p className='text-xs re1:text-[16px] max-h-[66px] line-clamp-2 re1:line-clamp-3 text-left re1:leading-[22px]'>{name}</p>
                <div>
                  <div>
                    <p className='text-sm font-bold text-[#dd1f26]'>por mais</p>
                    <span className='text-lg font-bold'>R$ {(price-(price*(parseFloat(obj.promotion)/100))).toFixed(2)}</span>
                  </div>
